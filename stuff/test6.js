...
//var cli1 = xmla.client('http://localhost:3000/xmla');
// var cli1 = xmla.client('http://sampledata.infragistics.com/olap/msmdpump.dll');
// var cli1 = xmla.client('http://localhost:8080/mondrian-embedded/xmla');


/*
cli1.discoverDataSources(function(err,rs){
	var ds = rs.rows[0].DataSourceName;
	cli1.discoverDBCatalogs(function(err,rs){
		var catalog = rs.rows[0].CATALOG_NAME;
		cli1.discoverMDCubes({CATALOG_NAME:catalog},function(err,rs){
			var cube = rs.rows[0].CUBE_NAME;
			cli1.discoverMDMeasures({CATALOG_NAME:catalog,CUBE_NAME:cube},function(err,rs){
				var measures = _.pluck(rs.rows,"MEASURE_NAME");
				cli1.discoverMDHierarchies({CATALOG_NAME:catalog,CUBE_NAME:cube},function(err,rs){
					var hierarchies = _.pluck(rs.rows,"HIERARCHY_NAME");
					var mdx = 'SELECT [Measures].['+measures[0]+'] ON COLUMNS FROM ['+cube+']';
					console.log(mdx);
					cli1.execute(mdx,{Catalog:catalog,Format:'Multidimensional'},{},function(err,rs){
						console.log(rs);
					});
				});
			});
